...
public class SiteBasicArtifactTest extends Arquillian {
    ...
    private static final String SITE_BASIC_ARTIFACT_NAME = "SiteBasic.xml";
    private static final String DELETE_SITE_BASIC_TASK_NAME = "Delete Site Basic File";

    @Test
    public void when_create_SiteBasic_file_executed_successfully_then_the_artifact_is_generated_on_smrs() {
        ...
        final String siteBasicArtifactId = getNodeArtifactId(nodeMo);
        ...
        assertThat(dpsHelper).withManagedObject(CommonArtifactDefinitions.NODE_ARTIFACT_FDN, nodeMo.getFdn(), siteBasicArtifactId)
                ...
                .withAttributeValue("generatedLocation", smrsArtifactsDirectory + "/" + SITE_BASIC_ARTIFACT_NAME);

        Assertions.assertThat(smrsArtifactsDirectory.listFiles())
                .as("Generated Site Basic file was not generated as expected at smrs location: " + smrsArtifactsDirectory.getAbsolutePath())
                .extracting("name")
                .contains(SITE_BASIC_ARTIFACT_NAME);

    }

    @Test
    public void when_create_SiteBasic_file_failed_then_artifact_not_found_exception_is_thrown() {
        final ProjectDescriptor projectDescriptor = createErbsProjectWithOneNode().without_site_basic_artifact()
                .build();

        final ManagedObject nodeMo = dataSteps.create_project_mo_and_return_node_mo(projectDescriptor);

        verifyException(workflowTaskEjbTestSteps).create_generated_artifact(ArtifactType.SITEBASIC.toString(), nodeMo.getFdn());
    ...
    @Test
    public void when_delete_SiteBasic_file_executed_successfully_then_the_artifact_is_deleted_from_smrs() {
        ...
        // CReate another node artifact in smrs so that the smrs directory doesn't also get removed at site basic deletion
        workflowTaskEjbTestSteps.create_generated_artifact(ArtifactType.SITEEQUIPMENT.toString(), nodeMo.getFdn());
        // Create the site basic so we can then delete it..
        workflowTaskEjbTestSteps.create_generated_artifact(ArtifactType.SITEBASIC.toString(), nodeMo.getFdn());

        // Execute the test method
        workflowTaskEjbTestSteps.delete_generated_artifact(ArtifactType.SITEBASIC.toString(), nodeMo.getFdn());

        // Verify the status updates for the ejb call
        final String taskStatus = dataSteps.get_ap_task_status(nodeMo.getFdn(), DELETE_SITE_BASIC_TASK_NAME);
        ...
        Assertions.assertThat(smrsArtifactsDirectory.listFiles())
                .extracting("name")
                .doesNotContain(SITE_BASIC_ARTIFACT_NAME)
                .as("Site Basic file was not deleted as expected from smrs location: " + smrsArtifactsDirectory.getAbsolutePath());
    ...
    private String getNodeArtifactId(final ManagedObject nodeMo) {
        final ManagedObject siteEquipmentArtifactMo = dataSteps.getNodeArtifactMo(nodeMo.getFdn(), ArtifactType.SITEBASIC.toString());
